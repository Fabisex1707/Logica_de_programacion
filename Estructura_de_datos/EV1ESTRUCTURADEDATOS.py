...
def registrar_cliente(diccionario_cliente: dict):
    while True:
        ...
        try:
            ...
            # Validación para que solo se permitan letras y espacios
            if not nombre or not apellido:
                raise ValueError("El nombre y los apellidos no deben estar vacíos.")
            if not nombre.replace(" ", "").isalpha() or not apellido.replace(" ", "").isalpha():
                print("El nombre y los apellidos solo deben contener letras y espacios.")
                continue
